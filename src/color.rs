...
impl ops::Mul<RT> for RRgb {
    type Output = RRgb;

    fn mul(self, rhs: RT) -> Self::Output {
        RRgb {
            r: (self.r as RT * rhs) as usize,
            g: (self.g as RT * rhs) as usize,
            b: (self.b as RT * rhs) as usize,
        }
    }
}
...
impl std::iter::Sum for RRgb {
    fn sum<I: Iterator<Item = RRgb>>(iter: I) -> Self {
        iter.fold(RRgb { r: 0, g: 0, b: 0 }, |rc, acc| rc + acc)
    }
}
...
impl From<RRgb> for Rgb<CT> {
    fn from(rrgb: RRgb) -> Self {
        let r = if rrgb.r > u8::MAX as usize {
            u8::MAX
        } else {
            rrgb.r as u8
        };
        let g = if rrgb.g > u8::MAX as usize {
            u8::MAX
        } else {
            rrgb.g as u8
        };
        let b = if rrgb.b > u8::MAX as usize {
            u8::MAX
        } else {
            rrgb.b as u8
        };
